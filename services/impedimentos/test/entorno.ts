 ...
 * versionado: apuntan al Supabase local con el rol `svc_impedimentos` y
 * `?schema=impedimentos`. Cualquier variable ya presente en el entorno (CI,
 * o `.env.test` cargado con `--env-file-if-exists`) tiene prioridad.
 */
process.env.DATABASE_URL ??=
  'postgresql://svc_impedimentos:svc_impedimentos_local_dev@127.0.0.1:54322/postgres?schema=impedimentos';
